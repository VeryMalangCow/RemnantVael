...
public class AllyExplosionController : ExplosionController
{
    #region Value
    ...
    public override void Set_State_Extra()
    {
        ...
        for (int i = 0; i < atList.Count; i++)
        {
            if (state.Get_AttributeCondition()[i])
            {
                ...
                DevTool.Set_Anim(ref Get_IndexAOC(i), atList[i], StaticResourceManager.instance.ExplosionReso.attributeExplosionAnimations[i]);
